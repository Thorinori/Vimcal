pub mod input_processing{
    ...
    use vosk::Recognizer;
    use crate::global_state::global_state::GLOBAL_STATE;
    ...
    pub fn process_input<T: Sample + ToSample<i16>>(recognizer: &mut Recognizer, data: &[T], channels: ChannelCount){
        ...
        match state {
            ...
            vosk::DecodingState::Finalized => {
                ...
                if GLOBAL_STATE.get().read().unwrap().listen{
                    if text.eq("quit") || text.eq("exit"){
                        println!("Exiting Vimcal...");   
                        GLOBAL_STATE.get().write().unwrap().run = false;
                    }else if text.eq("disable listening"){
                        GLOBAL_STATE.get().write().unwrap().listen = false;
                    ...
                    GLOBAL_STATE.get().write().unwrap().listen = true; 
